...
- 蓝牙搜索：只有 H5 显式调用 `startDiscovery` 时才请求。Android 12+ 使用 `BLUETOOTH_SCAN` 和 `ACCESS_FINE_LOCATION`；Android 6-11 使用 `BLUETOOTH`、`BLUETOOTH_ADMIN` 和 `ACCESS_FINE_LOCATION`。扫描附近 BLE 设备时要求系统定位服务处于开启状态。
- 二维码扫码：`CAMERA`。
